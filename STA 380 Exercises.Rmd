...
# Git Hub Link: [https://github.com/bohnemic/STA380Summer2021/blob/main/STA%20380%20Exercises.Rmd]
...
Based on our analyses, the excel guru's projections are inaccurate because he does not consider the confounding variable of building quality. To create a more accurate projection, he should consider green vs. non-green buildings in each class separately. It is impossible to make an accurate projection of rent revenue of a green vs non green building without knowing the quality of the building that is being built. Regardless, the spread among the green and non-green rent prices in their respective classes is much smaller than the spread in all of the buildings considered together. Therefore, it will probably take longer for the building to cover the 5 million extra dollars it would cost to have the building certified as green. 
...
# Exercise 3
Given that we had 4 ETFs in each of our 3 portfolios, we weighted them by assigning an equal weight of 0.25 to each ETF. This way each of our portfolios totaled to have a weight of 1.00 as they should.

As a note, due to the fluctuating nature of the stock market, our VaR values and Mean Profit or Loss are approximate and were recorded at the time the code chunk was run. What may have been our best portfolio at the time we conducted this analysis could change at some point in the future. 
...
In the C50train directory, you have 50 articles from each of 50 different authors (one author per directory). Use this training data (and this data alone) to build the model. Then apply your model to predict the authorship of the articles in the C50test directory, which is about the same size as the training set. 

The difficulty in this section lay in preprocessing the data, due to the number of txt files that were being used. First, the files were loaded in, and the files names were read in using readPlain(). Next, in order to make the text itself homogenous, adjustments were needed to be made to the text. Using the corpus function, the text was made to be all lowercase, and numbers, punctuation, white spaces, and stop words were removed from the text files. Next, sparse terms were also removed by using the removeSparseTerms function. This method was used on both the test and train sets. Once completed, in order to prepare the data sets for PCA, dimensionality reduction was necessary. This step removes the null columns and selects only the intersecting columns. Once this step had been completed, PCA could be run on the data set. PCA was used in order to determine the necessary amount of components tht could be sued to explain the majority of the data set. That information could then be used to run the three selected models, Naive Bayes, KNN, and Random Forest.
